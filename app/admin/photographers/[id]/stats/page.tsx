...
import { Calendar, TrendingUp, TrendingDown, ArrowLeft, Award, Info } from 'lucide-react';
...
export default function PhotographerStatsPage() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      setCurrentMonthStats({
        ...
        coursesDetails: currentMonthData?.coursesDetails || [],
      });
      ...
      setPreviousMonthStats({
        ...
        coursesDetails: previousMonthData?.coursesDetails || [],
      });
      ...
      for (let i = 1; i <= 12; i++) {
        ...
        months.push({
          ...
          coursesDetails: monthData?.coursesDetails || [],
        });
      }
  ...
  const backUrl = user?.isAdmin
    ? `/admin/admins/${photographerId}/profile`
    : `/admin/photographers/${photographerId}/profile`;
    ...
    <div className="h-full overflow-y-auto space-y-6 pb-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold tracking-tight">
            Statistiques - {user?.prenom} {user?.nom}
          </h1>
          {user?.isNonPaidAdmin && (
            <Badge variant="outline" className="text-orange-700 border-orange-300 bg-orange-50">
              <Info className="h-3 w-3 mr-1" />
              Admin non rémunéré (valeur estimée)
            </Badge>
          )}
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardDescription>{user?.isNonPaidAdmin ? "Valeur estimée" : "Revenus"}</CardDescription>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card className="bg-gradient-to-br from-gray-500/10 to-gray-600/20 dark:from-gray-500/20 dark:to-gray-600/30 border-gray-500/30">
            <CardHeader className="pb-3">
              <CardDescription>
                {user?.isNonPaidAdmin ? "Valeur totale estimée" : `Revenus totaux ${new Date().getFullYear()}`}
              </CardDescription>
      ...
      {/* Détail mois par mois avec détail des courses */}
      <div className="space-y-4">
            ...
            <Card key={index}>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between text-sm border-t pt-2 mt-2">
                  <span className="text-muted-foreground">{user?.isNonPaidAdmin ? "Valeur" : "Revenus"}</span>
                  <span className="font-bold text-gray-600">{formatCurrency(month.montantTotal)}</span>
                </div>

                {/* Détail des courses */}
                {month.coursesDetails.length > 0 && (
                  <div className="border-t pt-3 mt-3 space-y-2">
                    <div className="text-xs font-medium text-muted-foreground">Détail des courses :</div>
                    {month.coursesDetails.map((course, idx) => (
                      <div key={idx} className="text-xs bg-gray-50 dark:bg-gray-900 p-2 rounded">
                        <div className="font-medium">{course.nom}</div>
                        <div className="text-muted-foreground text-[10px]">
                          {course.ville} • {format(new Date(course.date), 'dd/MM/yyyy', { locale: fr })}
                        </div>
